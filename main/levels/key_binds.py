...
import globals
from globals import SCREEN_HEIGHT, SCREEN_WIDTH, LOBBY_MUSIC_PATH, FPS, KEY_BIND_SCREEN_PATH
import pygame

class KeyBinds(Level):
    def __init__(self, _name, _screen, _game_instance):
        super().__init__(_name, _screen, _game_instance)
        self.bg_image = pygame.image.load(KEY_BIND_SCREEN_PATH).convert_alpha()
